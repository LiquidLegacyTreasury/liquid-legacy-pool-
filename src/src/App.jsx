...
// Live Liquid Legacy Pool from Google Sheets (XRP 4% APY)
...
function Stat({ label, value, sub, monospace = false }) {
  return (
    <div className="rounded-2xl p-5 bg-zinc-900/60 border border-zinc-800 shadow-sm backdrop-blur">
      ...
      <div className={`mt-1 text-3xl md:text-4xl font-semibold ${monospace ? 'tabular-nums' : ''}`}>{value}</div>
...
export default function App() {
  ...
  useEffect(() => {
    const fetchCsv = async () => {
      try {
        ...
        if (!Number.isFinite(num)) throw new Error("Could not parse a numeric XRP amount.");
        setPoolXrpRaw(num);
        setSheetError("");
      } catch (e) {
  ...
  const annualYieldXrp = useMemo(() => (poolXrpRaw ? poolXrpRaw * FIXED_APY : 0), [poolXrpRaw]);
  ...
  const poolUsd = useMemo(() => (xrpUsd && poolXrpRaw ? poolXrpRaw * xrpUsd : null), [xrpUsd, poolXrpRaw]);
  const yearUsd = useMemo(() => (xrpUsd ? annualYieldXrp * xrpUsd : null), [xrpUsd, annualYieldXrp]);
  const fmt = (n, max = 2) => (n == null ? "—" : n >= 1000 ? n.toLocaleString(undefined, { maximumFractionDigits: 0 }) : n.toLocaleString(undefined, { maximumFractionDigits: max }));
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 [mask-image:radial-gradient(circle_at_center,black,transparent_70%)]">
        <div className="absolute -top-40 -left-40 h-[32rem] w-[32rem] rounded-full blur-3xl opacity-30" style={{ background: "radial-gradient(closest-side, #facc15, transparent)" }} />
        <div className="absolute -bottom-40 -right-40 h-[28rem] w-[28rem] rounded-full blur-3xl opacity-20" style={{ background: "radial-gradient(closest-side, #f59e0b, transparent)" }} />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-5 py-10 md:py-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <Stat label="Total Pool (XRP)" monospace value={<span className="text-yellow-400">{fmt(animatedPool, 2)} XRP</span>} sub={poolUsd != null ? `≈ $${fmt(poolUsd, 0)} USD` : "USD live price loading..."} />
          <Stat label="APY (fixed)" value={<span className="text-yellow-300">4.00%</span>} sub="Simple (non-compounding)" />
          <Stat label="Est. Annual Yield" monospace value={<span className="text-yellow-400">{fmt(animatedYear, 2)} XRP / yr</span>} sub={yearUsd != null ? `≈ $${fmt(yearUsd, 0)} USD / yr` : "USD live price loading..."} />
          <Stat label="Est. Monthly Yield" monospace value={<span className="text-yellow-400">{fmt(animatedMonth, 2)} XRP / mo</span>} sub="Annual ÷ 12" />
        </section>

        <div className="mt-8 text-xs text-zinc-500">* 4% APY simple calculation. For compounding: XRP × (1 + 0.04/12)^(12×years) − XRP.</div>

        <div className="mt-8 flex flex-wrap gap-3">
          <a href={SHEETS_CSV_URL} target="_blank" rel="noreferrer" className="px-4 py-2 rounded-xl border border-yellow-500/40 bg-yellow-500/10 hover:bg-yellow-500/20 transition">View Sheet Source</a>
          <button onClick={() => window.location.reload()} className="px-4 py-2 rounded-xl border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 transition">Refresh</button>
        </div>
